    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden group transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="p-4">
          ...
          <p className="text-slate-500 text-sm mt-2 h-10 overflow-hidden">{product.description}</p>
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xl font-extrabold text-primary">
              {formatPrice(product.price)}
              <span className="text-sm font-medium text-slate-500"> / {product.unit}</span>
            </p>
            <span className="text-sm text-green-600 font-medium">Stok: {product.stock}</span>
          </div>
        </div>
      </Link>
      <div className="p-4 bg-slate-50 border-t border-slate-200">
         <Link to={`/product/${product.id}`} className="w-full text-center bg-primary text-white font-semibold py-2 px-4 rounded-md hover:bg-primary-dark transition-colors duration-300 block">
            Lihat Detail
         </Link>
      </div>
    </div>
