...
const ProductDetailPage: React.FC = () => {
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <Link to="/catalog" className="text-primary hover:underline mt-4 inline-block">Kembali ke Katalog</Link>
      </div>
    );
  }
  ...
  const handleQuoteSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Permintaan penawaran untuk ${quantity} ${product.unit} ${product.name} telah diterima. Kami akan segera menghubungi Anda.`);
  };
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[product.image, ...product.gallery].slice(0, 4).map((img, index) => (
                <button
                  ...
                  className={`block border-2 rounded-md overflow-hidden ${mainImage === img ? 'border-primary' : 'border-transparent'}`}
                >
          ...
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-secondary">{product.name}</h1>
            <p className="text-3xl font-bold text-primary mt-4">
              {formatPrice(product.price)}
              <span className="text-lg font-medium text-slate-500"> / {product.unit}</span>
            </p>
            <p className="mt-4 text-slate-600 leading-relaxed">{product.description}</p>
            
            <div className="mt-6">
                <h3 className="text-lg font-semibold text-secondary mb-2">Spesifikasi</h3>
                <ul className="border-t border-slate-200">
                    {Object.entries(product.specifications).map(([key, value]) => (
                        <li key={key} className="flex justify-between py-2 border-b border-slate-200">
            ...
            {/* Quote Form */}
            <form onSubmit={handleQuoteSubmit} className="mt-8 p-6 bg-slate-50 rounded-lg border border-slate-200">
              <h3 className="text-xl font-bold text-secondary mb-4">Minta Penawaran</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="quantity" className="block text-sm font-medium text-slate-700">Jumlah</label>
                 ...
                 <div>
                  <label htmlFor="name" className="block text-sm font-medium text-slate-700">Nama Anda</label>
                 ...
                 <div>
                  ...
                  <input type="tel" id="phone" required className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:ring-primary focus:border-primary sm:text-sm" />
                </div>
                 <div>
