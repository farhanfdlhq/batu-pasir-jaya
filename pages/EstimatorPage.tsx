...
const EstimatorPage: React.FC = () => {
  ...
  const [estimation, setEstimation] = useState<EstimatedMaterial[] | null>(null);
  ...
  const handleEstimate = async () => {
    ...
    try {
      const result = await estimateMaterials(projectDescription);
    ...
    <div className="bg-white py-12">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-secondary">AI Material Estimator</h1>
          <p className="mt-4 text-lg text-slate-600">
            Jelaskan proyek Anda, dan biarkan AI kami memberikan perkiraan kebutuhan material untuk Anda.
          </p>
        </div>

        <div className="bg-slate-50 p-8 rounded-lg shadow-md border border-slate-200">
          <div className="mb-4">
            <label htmlFor="project-description" className="block text-lg font-semibold text-slate-800 mb-2">
        ...
        {estimation && (
          <div className="mt-10">
            <h2 className="text-2xl font-bold text-secondary mb-4">Hasil Estimasi AI</h2>
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        ...
                        <tr key={index}>
                            ...
                            <div className="text-sm font-medium text-secondary">{item.materialName}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-slate-900">{item.quantity} {item.unit}</div>
                            </td>
                            <td className="px-6 py-4">
                            <p className="text-sm text-slate-600 max-w-xs break-words">{item.notes}</p>
                            </td>
                        </tr>
                        ))}
                    </tbody>
             ...
             <p className="mt-4 text-sm text-slate-500 text-center">
                *Estimasi ini dihasilkan oleh AI dan bersifat perkiraan. Kebutuhan aktual dapat bervariasi tergantung kondisi lapangan dan metode kerja.
            </p>
          </div>
        )}
      </div>
